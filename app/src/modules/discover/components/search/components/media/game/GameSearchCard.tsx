...
export const GameSearchCard = ({ item }: GameSearchCardProps) => {
  return (
    ...
    >
      <span className="bg-foreground/5 text-muted flex aspect-3/4 w-full items-center justify-center overflow-hidden">
        {item.coverUrl ? (
      ...
      <div className="space-y-0.5 p-3">
        ...
        {item.year !== undefined && <p className="text-muted text-xs tabular-nums">{item.year}</p>}
      </div>
    </Link>
  )
}
