...
export const Avatar = ({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        'group/avatar relative flex size-8 shrink-0 rounded-full select-none after:absolute after:inset-0 after:rounded-full after:border after:border-border after:mix-blend-darken data-[size=lg]:size-10 data-[size=xl]:size-12 data-[size=sm]:size-6 dark:after:mix-blend-lighten',
        className,
      )}
      {...props}
    />
...
export const AvatarFallback = ({ className, ...props }: AvatarPrimitive.Fallback.Props) => {
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn(
        'flex size-full items-center justify-center rounded-full bg-muted text-sm text-muted-foreground group-data-[size=xl]/avatar:text-base group-data-[size=sm]/avatar:text-xs',
        className,
      )}
      {...props}
    />
